<template>
  ...
  >
    <div :class="buttonWrapClass">
      ...
      >
        <v-icon v-if="val === -1 && deleteKeyIcon"> {{ deleteKeyIcon }}</v-icon>
        <template v-else>
          {{ showKey(val) }}
        </template>
      </button>
...
export default {
  ...
  props: {
    ...
    options: {
      ...
      validator: function (value) {
        ...
        if (keyArrayDisable) {
          console.error(
            "KeyArray can contain only an integer 'number' between -2 and 9 and an empty 'string'."
          );
          return false;
        }
        const classDisable = Object.keys(value).some(
          (key) => /Class/.test(key) && /[^0-9A-z\-_ ]/.test(value[key])
        );
        if (classDisable) {
          console.error(
            "Class name can contain only 'a-z' and 'A-Z', '0-9', '_', '-', ' '."
          );
          return false;
        }
        return true;
  ...
  data() {
    ...
    return {
      ...
      encryptedChar:
        typeof this.options.encryptedChar === "string"
          ? this.options.encryptedChar.substring(0, 1)
          : "0",
      keyArray: this.options.keyArray || (columns === 3 ? arr1 : arr2),
      keyRandomize: Boolean(
        this.options.keyRandomize === undefined || this.options.keyRandomize
      ),
      randomizeWhenClick: Boolean(this.options.randomizeWhenClick),
      fixDeleteKey: Boolean(
        this.options.fixDeleteKey === undefined || this.options.fixDeleteKey
      ),
      fixBlankKey: Boolean(
        this.options.fixBlankKey === undefined || this.options.fixBlankKey
      ),
      stopPropagation: Boolean(
        this.options.stopPropagation === undefined ||
          this.options.stopPropagation
      ),
      keypadClass: this.options.keypadClass || "numeric-keypad",
      buttonWrapClass:
        this.options.buttonWrapClass || "numeric-keypad__button-wrap",
      buttonClass: this.options.buttonClass || "numeric-keypad__button",
      deleteButtonClass:
        this.options.deleteButtonClass || "numeric-keypad__button--delete",
      clearButtonClass:
        this.options.clearButtonClass || "numeric-keypad__button--clear",
      blankButtonClass:
        this.options.blankButtonClass || "numeric-keypad__button--blank",
      activeButtonClass:
        this.options.activeButtonClass || "numeric-keypad__button--active",
      activeButtonIndexes: {},
      activeButtonDelay: Number(this.options.activeButtonDelay) || 300,
      pseudoClick: Boolean(this.options.pseudoClick),
      pseudoClickDeleteKey:
        this.options.pseudoClickDeleteKey === undefined
          ? Boolean(this.options.pseudoClick)
          : Boolean(this.options.pseudoClickDeleteKey),
      pseudoClickBlankKey:
        this.options.pseudoClickBlankKey === undefined
          ? Boolean(this.options.pseudoClick)
          : Boolean(this.options.pseudoClickBlankKey),
      ...
      defaultStyle:
        ["all", "button", "wrap", "none"].find(
          (s) => s === this.options.defaultStyle
        ) || "all",
      keypadStylesIndex: null,
      deleteKeyText:
        this.options.deleteKeyText === undefined
          ? "del"
          : this.options.deleteKeyText,
      deleteKeyIcon: this.options.deleteKeyIcon || "",
      clearKeyText:
        this.options.clearKeyText === undefined
          ? "clr"
          : this.options.clearKeyText,
    };
  },
  watch: {
    show: function () {
    ...
    options: {
      ...
      handler(options) {
        ...
        this.encryptedChar =
          typeof options.encryptedChar === "string"
            ? options.encryptedChar.substring(0, 1)
            : "0";
        this.keyArray = options.keyArray || (columns === 3 ? arr1 : arr2);
        this.keyRandomize = Boolean(
          options.keyRandomize === undefined || options.keyRandomize
        );
        this.randomizeWhenClick = Boolean(options.randomizeWhenClick);
        this.fixDeleteKey = Boolean(
          options.fixDeleteKey === undefined || options.fixDeleteKey
        );
        this.fixBlankKey = Boolean(
          options.fixBlankKey === undefined || options.fixBlankKey
        );
        this.stopPropagation = Boolean(
          options.stopPropagation === undefined || options.stopPropagation
        );
        this.activeButtonDelay = Number(options.activeButtonDelay) || 300;
        this.pseudoClick = Boolean(options.pseudoClick);
        this.pseudoClickDeleteKey =
          options.pseudoClickDeleteKey === undefined
            ? Boolean(options.pseudoClick)
            : Boolean(options.pseudoClickDeleteKey);
        this.pseudoClickBlankKey =
          options.pseudoClickBlankKey === undefined
            ? Boolean(options.pseudoClick)
            : Boolean(options.pseudoClickBlankKey);
        ...
        this.deleteKeyText =
          options.deleteKeyText === undefined ? "del" : options.deleteKeyText;
        this.clearKeyText =
          options.clearKeyText === undefined ? "clr" : options.clearKeyText;
        const defaultStyle =
          ["all", "button", "wrap", "none"].find(
            (s) => s === options.defaultStyle
          ) || "all";
  ...
  computed: {
    keypadStyles: function () {
      const fontSize = Math.round(
        Math.min(this.cellWidth, this.cellHeight) * 0.3
      );
      const style = `
        position: fixed;
        z-index: ${this.zIndex};
    ...
    buttonStyles: function () {
      const width = `calc(${
        Math.round(1000 / this.columns) / 10
      }% - ${Math.ceil((5 * (this.columns - 1)) / this.columns)}px)`;
      const height = `calc(${Math.round(1000 / this.rows) / 10}% - ${Math.ceil(
        (5 * (this.rows - 1)) / this.rows
      )}px)`;
      return `
        flex: 0 0 auto;
        display: flex;
  ...
  methods: {
    click(key, idx, event) {
      ...
      if (this.pseudoClick) {
        if (
          !(
            ((key == -1 || key == -2) && !this.pseudoClickDeleteKey) ||
            (key == "" && !this.pseudoClickBlankKey)
          )
        ) {
      ...
      if (this.vibrate && window.navigator.vibrate)
        window.navigator.vibrate(this.vibratePattern);
      switch (key) {
        case -1:
          this.del();
          break;
    ...
    randomize() {
      ...
      for (let i = 0; i < l; i++) {
        ...
        let count = 0;
        while (
          (this.checkFix(newkeyArray[r]) ||
            newkeyArray[r] === newkeyArray[i]) &&
          count < l
        ) {
          r = (r + 1) % l;
          count++;
    ...
    resize() {
      ...
      if (sheet && this.keypadStylesIndex !== null) {
        sheet.deleteRule(0);
        sheet.insertRule(
          `.${this.keypadClass.trim().split(" ")[0]} {${this.keypadStyles}}`,
          0
        );
      }
    },
    ...
    initDefaultStyles(sheet) {
      ...
      if (this.defaultStyle !== "button") {
        ...
        if (!test.test(this.buttonWrapClass)) {
          sheet.insertRule(
            `.${this.buttonWrapClass.trim().split(" ")[0]} {${
              this.buttonWrapStyles
            }}`,
            padIndex++
          );
        }
      ...
      if (this.defaultStyle !== "wrap") {
        if (!test.test(this.buttonClass)) {
          sheet.insertRule(
            `.${this.buttonClass.trim().split(" ")[0]} {${this.buttonStyles}}`,
            padIndex++
          );
          if (!test.test(this.activeButtonClass)) {
            sheet.insertRule(
              `.${this.buttonClass.trim().split(" ")[0]}.${
                this.activeButtonClass.trim().split(" ")[0]
